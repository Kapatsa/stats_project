...
```{r}
t.test(gdpgrowth ~ oecd, data = df)
wilcox.test(gdpgrowth ~ oecd, data = df)
```

```{r}
t.test(log(gdp85) ~ oil, data = df)
wilcox.test(log(gdp85) ~ oil, data = df)
```

```{r}
t.test(invest ~ oil, data = df)
t.test(invest ~ oil, data = df, alternative = "less")
```
