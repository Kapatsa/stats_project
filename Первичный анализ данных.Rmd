...
df_group$gdp60[ind_60] <- NA
df_group$gdp85[ind_85] <- NA
...
На первый взгляд можно предположить, что наблюдаются отличия во всех сравнениях.
...
```{r}
par(mfrow=c(2,2))
qqnorm(log(df_group$gdp60[which(df$oecd=="yes")]), pch = 1, frame = FALSE, col= "black", main = "1960", xlab = "log GDP (per capita)")

qqline(log(df_group$gdp60[df_group$oecd == "yes"]), col = "steelblue", lwd = 2)

qqnorm(log(df_group$gdp85)[which(df$oecd=="yes")],pch = 1, frame = FALSE, col= "black", main = "1985", xlab = "log GDP (per capita)")

qqline(log(df_group$gdp85[df_group$oecd == "yes"]), col = "steelblue", lwd = 2)

title("oecd members", line = -1, outer = TRUE)
qqnorm(log(df_group$gdp60[which(df$oecd=="no")]), pch = 1, frame = FALSE,col= "orange", main = "1960", xlab = "log GDP (per capita)")

qqline(log(df_group$gdp60[df_group$oecd == "no"]), col = "steelblue", lwd = 2)

qqnorm(log(df_group$gdp85)[which(df$oecd=="no")],pch = 1, frame = FALSE, col= "orange", main = "1985", xlab = "log GDP (per capita)")

qqline(log(df_group$gdp85[df_group$oecd == "no"]), col = "steelblue", lwd = 2)
...
table_pp <- data.frame(Data = c("log(gdp60))", "log(gdp85))","invest", "school"),
                      ...
                      Pearson = c(0.07117, 0.03387 ,0.004587 ,7.895e-05 )
)
...
По полученным результатам можно сделать следующие выводы:
1) Мы не можем принять гипотезу о нормальном распределении всех признаков.
